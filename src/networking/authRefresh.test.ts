...
/**
 * The gate every refresh path consults before rotating. It has to read
 * storage, not redux: `scrubSensitiveChatStateTransform` blanks the
 * tokens on the way into redux-persist, so a rehydrated tab holds an
 * empty refreshToken while localStorage still has the live one.
 */
describe('hasRotatableSession', () => {
  it('is true when only localStorage has the refresh token', () => {
    // Order matters: `setUser` persists, so redux has to be put into
    // its rehydrated (scrubbed) shape BEFORE the other tab's write.
    seedUser('', '');
    otherTabRotatedTo('access-2', 'refresh-2');

    expect(store.getState().chatSettingStore.user.refreshToken).toBe('');
    expect(hasRotatableSession()).toBe(true);
  });

  it('falls back to the redux copy when storage is empty', () => {
    clearStoredUser();
    seedUser('refresh-1');
  ...
  it('is false with no token anywhere', () => {
    clearStoredUser();
    seedUser('', '');

    expect(hasRotatableSession()).toBe(false);
  });

  it('is true with no token but a host refreshFunction', () => {
    clearStoredUser();
    seedUser('', '');
    store.dispatch(
      setConfig({
        refreshTokens: { enabled: true, refreshFunction: vi.fn() },
      } as never)
    );
...
/**
 * The tab that did NOT rotate still has to catch up, otherwise every
 * request it makes burns a 401 and a retry until it refreshes itself.
 */
describe('cross-tab token sync', () => {
  const fireStorageEvent = (key: string | null, newValue: string | null) => {
    window.dispatchEvent(
      new StorageEvent('storage', { key, newValue } as StorageEventInit)
    );
  };

  const sessionKey = '@ethora/chat-component-user-session';

  beforeEach(() => {
    // Idempotent: the module installs the listener at load, this just
    // makes the dependency explicit for the cases below.
    startCrossTabTokenSync();
  });

  it('adopts tokens another tab wrote', () => {
  ...
  it('ignores events for other keys', () => {
    ...
    fireStorageEvent('some-other-key', 'whatever');
  ...
  it('ignores a cleared key instead of wiping the session', () => {
    seedUser('refresh-1', 'access-1');

    fireStorageEvent(sessionKey, null);
  ...
  it('does not adopt while this tab is mid-rotation', async () => {
    let releaseRotation: (value: unknown) => void = () => undefined;
    post.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
    ...
    const rotating = refreshAuthTokens();
    await Promise.resolve();

    // Another tab's (older) write lands mid-flight.
    otherTabRotatedTo('access-stale', 'refresh-stale');
    fireStorageEvent(sessionKey, localStorage.getItem(sessionKey));
    expect(store.getState().chatSettingStore.user.refreshToken).not.toBe(
      'refresh-stale'
    );

    releaseRotation({ data: { token: 'access-3', refreshToken: 'refresh-3' } });
